...
using.test_results=metab.test.results
...
metab.kk2=as.data.frame(kk2)
metab.kk2=metab.kk2[order(-abs(metab.kk2$enrichmentScore)),]
gseaplot(kk2, by = "preranked", title = kk2$Description[1], geneSetID = 1)
head(kk2)

description_name="Regulation of lipolysis in adipocytes"
wanted_pathways=metab.kk2[abs(metab.kk2$NES)>2.5,'Description']
...
using.test_results = lipid.test.results
using.test_results$fc=2^(using.test_results$log2fc)
foldchanges=using.test_results[,"fc"]
names(foldchanges)=using.test_results[,'ENTREZID']
# gene_list<-na.omit(gene_list)
...
lipid.kk2=as.data.frame(kk2)
lipid.kk2=lipid.kk2[order(-abs(lipid.kk2$enrichmentScore)),]
gseaplot(kk2, by = "runningScore", title = kk2$Description[1], geneSetID = 1)
head(kk2)

wanted_pathways=lipid.kk2[abs(lipid.kk2$NES)>2.5,'Description']

for (description_name in wanted_pathways){
  p=gseaplot2(kk2,title = description_name, geneSetID = which(kk2$Description==description_name))
  
  filepath_name=paste0('~/workstation/MetabSubtype/tasks/MultiOmics/results/20240930_essay_figures/gsea/lipid/',description_name,'.pdf')
  pdf(filepath_name)
  print(p)
  dev.off()
...
# View available references
SingleR::listDatasets()

# For example, load the Human Primary Cell Atlas
...
tcell_markers <- hpca.se$labels[hpca.se$labels == "T cells"]
```
