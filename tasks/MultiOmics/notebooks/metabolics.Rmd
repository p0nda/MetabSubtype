...
dirpath='~/workstation/MetabSubtype/tasks/MultiOmics/results/20240930_essay_figures/'
```
...
df.use=merge(df.raw_metab.scaled,df.cluster_result[,class_label,drop=FALSE],by="row.names")
...
```{r}
rcc = ConsensusClusterPlus(as.matrix(df.raw_metab.scaled), 
                           maxK = 8, 
                           reps = 100, 
                           pItem = 0.8, 
                           pFeature = 1,
...
feature_cols=filter_by_foldchange(df.use,colnames(df.use)[1:(ncol(df.use)-1)],class_label,log2(1.5))
feature_cols=filter_by_kruskal(df.use,colnames(df.use)[1:(ncol(df.use)-1)],class_label,1e-2)



filepath=paste0(dirpath,'metab_sig_heatmap_1e-2.pdf')
pdf(filepath,10,5)
draw_single_heatmap(df.use,feature_cols,'kmeans_2_clusters',FALSE )
dev.off()

filepath=paste0(dirpath,'metab_all_heatmap.pdf')
pdf(filepath)
p=draw_single_heatmap(df.use,metab_num,'kmeans_2_clusters',FALSE )
dev.off()

# draw_single_heatmap(df.use,metab_num,'nmf_2_clusters',p_cutoff,FALSE )
feature_cols=colnames(df.raw_metab)
class_label='kmeans_2_clusters'
type2=2
type1=1
  df.foldchange <- data.frame(Metabolites = feature_cols)
  df.foldchange$log2FC <- apply(df.raw_metab[, feature_cols], 2, function(x) {
    log2(mean(x[df.use[[class_label]] == type2]) / 
           mean(x[df.use[[class_label]] == type1]))
  })
```

```{r}
mat=df.use[,c(feature_cols,class_label)]
p = Heatmap(as.matrix(t(mat[,feature_cols])) , width = unit(10, "cm"),
                     km = 12, 
                     cluster_columns = F, 
                     show_row_names = F, 
                     row_title_rot = 0, 
                     ...
                     col = mat[,class_label])
p=draw_single_heatmap(df.use,feature_cols,'kmeans_2_clusters',FALSE )
p = draw(p)
rcl.list <- row_order(p)
clu_df <- do.call(rbind, lapply(1:length(rcl.list), function(i){
  out <- data.frame(GeneID = colnames(df.use[rcl.list[[i]],]),
...
my_hclust_gene <- hclust(dist(t(df.use[,feature_cols])), method = "complete")

as.dendrogram(my_hclust_gene) %>%
  plot(horiz = TRUE)

my_gene_col <- cutree(tree = as.dendrogram(my_hclust_gene), k = 20)
my_gene_col <- data.frame(cluster = my_gene_col)
my_gene_col
my_gene_col[order(my_gene_col$cluster),]
```
...
row.names(df.use)=df.use[[1]]
df.use=df.use[,2:ncol(df.use)]
# 假设你的数据框是 loaddata，行是基因，列是样本
# df.sample_cluster 包含样本的聚类信息
cluster2_samples <- df.use$kmeans_2_clusters == 2
cluster1_samples <- df.use$kmeans_2_clusters == 1


# Calculate mean
mean_expr_cluster1 <- colMeans(df.use[cluster1_samples, 1:(ncol(df.use)-1)],na.rm=TRUE)
mean_expr_cluster2 <- colMeans(df.use[cluster2_samples,1:(ncol(df.use)-1)] ,na.rm=TRUE)

diff_expr <- mean_expr_cluster2 /mean_expr_cluster1

upregulated_genes <- colnames(df.use[,1:(ncol(df.use)-1)])[abs(diff_expr) > 1]

# upregulated_genes <- colnames(loaddata)[abs(diff_expr) > 1]
...
df.test.results.mt2=kegg_before_process(df.use,metab_num,class_label,types=c('2','1'))
# Key process to control upregulate
df.test.results.mt2=df.test.results.mt2[df.test.results.mt2$log2FC>0,]
kegg_table.up=kegg_after_process(df.test.results.mt2,pvalue_cutoff,fc_up_cutoff,fc_down_cutoff)
kegg_table


# Draw Table
kegg_table_draw=kegg_table.up
...
print(p)
# write.csv(kegg_table.up,"~/workstation/MetabSubtype/tasks/MultiOmics/results/20240924_external_genes/metab_up_kegg.csv")
kegg_table
```
...
# Key process to control upregulate
df.test.results.mt2=df.test.results.mt2[df.test.results.mt2$log2FC<0,]
kegg_table.down=kegg_after_process(df.test.results.mt2,pvalue_cutoff,fc_up_cutoff,fc_down_cutoff)
kegg_table


# Draw Table
kegg_table_draw=kegg_table.down
...
print(p)
# write.csv(kegg_table.down,"~/workstation/MetabSubtype/tasks/MultiOmics/results/20240924_external_genes/metab_down_kegg.csv")
kegg_table
```
...
# I upregulated too much
kegg_table.down=kegg_table.down[order(kegg_table.down$pvalue,decreasing = TRUE),]
# kegg_table.down=kegg_table.down[(nrow(kegg_table.down)-3):nrow(kegg_table.down),]
# II upregulated too much
kegg_table.up=kegg_table.up[order(kegg_table.up$pvalue,decreasing = TRUE),]
kegg_table.up=kegg_table.up[(nrow(kegg_table.up)-6):nrow(kegg_table.up),]

kegg_table.up_down=rbind(kegg_table.up,kegg_table.down)
kegg_table_draw=kegg_table.up_down
unwanted_pathways=c('Glycerolipid metabolism','Butanoate metabolism','Lipoic acid metabolism','Arginine biosynthesis','Nitrogen metabolism')
kegg_table_draw=kegg_table_draw[!kegg_table_draw$Description %in% unwanted_pathways,]
names(kegg_table_draw)[names(kegg_table_draw) == "p.adjust"] <- "FDR"
p=ggplot(kegg_table_draw, aes(comporison, Description)) +
  geom_point(aes(fill = -log10(FDR), size = FoldEnrich), color = "black", shape = 21) +
  scale_y_discrete(limits = kegg_table_draw[['Description']])+
  scale_size(range = c(3, 18), breaks = c(seq(0,10,2))) +
  # scale_color_gradient(low="blue",high = "red")+
  scale_fill_viridis_c(option = "A", direction = -1, begin = 0.4, breaks = c(seq(1,3,0.5)))+
  #  scale_fill_viridis_c(option = "A", direction = -1, begin = 0.4, breaks = c(0.2,0.5, 1, 1.5)) +
  theme(plot.margin = margin(1, 1, 1, 1, "cm")) +
  ...
  theme(legend.text = element_text(size = 10, face = "plain", colour = "black"), 
        legend.title = element_text(size = 10, face = "plain", colour = "black"), 
        legend.key.height = unit(0.3, "cm"), legend.key.width = unit(0.3, "cm"))
pdf('~/workstation/MetabSubtype/tasks/MultiOmics/results/20240930_essay_figures/metab_I_II_upregulated.pdf',12,10)
print(p)
dev.off()
```
...
##### Mean Aggregate #####  
dirpath="~/workstation/MetabSubtype/tasks/MultiOmics/results/20240930_essay_figures/metab_pathway_heatmap/"
# setwd(dirpath)
class_label='kmeans_2_clusters'
...
metabolite_map <- list(
  ...
  "d-glyceraldehdye-3-phosphate" = "G3P",
  "d-sedoheptulose-1-7-phosphate" = "S7P",
  "dtmp_neg" = "dTMP",
  "dump" = "dUMP",
  ...
  "hexose-phosphate" = "HexP",
  "homocysteine" = "Hcy",
  "hydroxyproline" = "Hyp",
  "imp" = "IMP",
  "kynurenine" = "Kyn",
  "n-acetyl-glucosamine-1-phosphate" = "GlcNAc-1P",
  "nicotinamide ribotide" = "NMN",
  "o-acetyl-l-serine" = "O-Ac-Ser",
  "pantothenate" = "Pant",
  "phenyllactic acid" = "PhLA",
  "phosphoenolpyruvate" = "PEP",
  "quinolinate" = "QA",
  "s-adenosyl-l-homocysteine_neg" = "SAH",
  "s-adenosyl-l-homocysteine_pos" = "SAH",
...
# 读取代谢物文件
# metabolite_file <- "path_to_your_metabolite_file.txt"
# metabolites <- readLines(metabolite_file)

# 匹配代谢物名称
standardized_metabolites <- sapply(volcano.test.results$Metabolites, function(met) {
  if (met %in% names(metabolite_map)) {
    return(metabolite_map[[met]])
  } else {
    return(met)  # 如果找不到则保留原名称
  }
})

# 输出匹配结果
volcano.test.results$std_Metabolites=sapply(volcano.test.results$Metabolites, function(met) {
  if (met %in% names(metabolite_map)) {
    return(metabolite_map[[met]])
  } else {
    return(met)  # 如果找不到则保留原名称
...
pdf("~/workstation/MetabSubtype/tasks/MultiOmics/results/20240930_essay_figures/metab_volcano.pdf")
draw_volcano(volcano.test.results,pvalue_cutoff,fc_up_cutoff,use_std=TRUE)
dev.off()
```
...
filepath=paste0(dirpath,'metab_os.pdf')
pdf(filepath)
print(kmeans_plot)
dev.off()
# NMF
