...
# df.raw_metab.log=log2(df.raw_metab)
# df.raw_metab.scaled=df.raw_metab.log
# raw_metab_mean=apply(df.raw_metab.log,2,mean)
# raw_metab_std=apply(df.raw_metab.log,2,sd)
# df.raw_metab.scaled=(df.raw_metab.scaled-raw_metab_mean)/raw_metab_std

mean(df.raw_metab.scaled[, 1])
mean(as.numeric(df.raw_metab.scaled[1, ]))
first_col_df <- data.frame(Metabolite = df.raw_metab.scaled[, 5])
first_row_df <- data.frame(Metabolite = as.numeric(df.raw_metab.scaled[5, ]))

# Plot the distribution of the first column
ggplot(first_col_df, aes(x = Metabolite)) +
  geom_density(fill = "blue", alpha = 0.5) +  # Density plot
  labs(title = "Distribution of the First Column", x = "Values", y = "Density")

ggplot(first_row_df, aes(x = Metabolite)) +
...
filepath.cluster='~/workstation/MetabSubtype/tasks/Subtype/data/lipid/cluster_result.csv'
df.cluster_result.correction
#
