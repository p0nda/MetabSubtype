...
filepath.cluster_result='/home/suh/workstation/MetabSubtype/tasks/MultiOmics/data/lipid/cluster_result.csv'
...
```{r}
df.using=merge(df.raw_counts,df.sample[,(ncol(df.sample)-2):ncol(df.sample)],by="row.names")
rownames(df.using)=df.using[,1]
df.using=df.using[,2:ncol(df.using)]
df.using = df.using[!is.na(df.using[class_label]),]
...
```{r}
df.sample[df.sample$`Sample Name`=='7038',]
...
##### Survival #####
df.use.os=df.sample[,c(class_label,time_label_col,event_label_col)]
...
# K-MEANS
kmeans_fit=survfit(surv_object~lipid_cluster,data=df.use.os)
...
# batch_corrected_log <- removeBatchEffect(log_counts, batch = batch, design = model.matrix(~group))
# batch_corrected_counts <- 2^batch_corrected_log - 1
...
lipid.test.results=glmLRT(fit)$table
logFC <- glmLRT(fit)$table$logFC
dim(v$E)
v_corrected.edger=as.data.frame(t(v$E))
rna_num=ncol(v_corrected.edger)

mean_group1 <- mean(df.using[df.using[class_label]==1,'ENSG00000187288' ])
mean_group2 <- mean(df.using[df.using[class_label]==2,'ENSG00000187288' ])
mean_group1
mean_group2

log2(mean_group2/mean_group1)
lipid.test.results %>%
  filter(rownames(lipid.test.results) == 'ENSG00000187288')
# write.csv(v_corrected.edger,'~/workstation/MetabSubtype/tasks/MultiOmics/data/rna/edger.csv')
```

```{r}
bitr('ENSG00000187288', #数据集
...
all_genes=colnames(df.raw_counts)
...
```{r}
class_label='kmeans_2_clusters'
df.cluster_result[class_label]=as.factor(df.cluster_result[[class_label]])
genes = c(
    "HK1", "HK2", "HKDC1", "PFKP", "PFKL", "FBP1", "FBP2", 
    "ALDOA", "ALDOB", "ALDOC", "GPI", "GAPDH", "PGK1", 
    "ENO1", "ENO2", "ENO3", "PKM", "PDP1", "PDP2", 
    "AKR1B1", "ADH1A", "ADH1B", "ADH1C", "ADH4", "ADH5", 
    "ACSS1", "ACSS2", "TALDO1", "PGD", "TKT", "TALDO1", 
    "RPE", "RPIA", "PRPS1", "PRPS2", "G6PD", "GLYCTK"
)
test = bitr(genes, #数据集
  fromType="SYMBOL", #输入为SYMBOL格式
  toType="ENSEMBL",  # 转为ENTERZID格式
...
df.sample_cluster$kmeans_2_clusters=as.factor(df.sample_cluster[['kmeans_2_clusters']])
feature_cols=test[['ENSEMBL']]
shared_cols=intersect(colnames(v_corrected.edger), feature_cols)

loaddata=v_corrected.edger[,shared_cols]
colnames(loaddata) = test[match(shared_cols, test[['ENSEMBL']]), 'SYMBOL']
loaddata=merge(loaddata,df.cluster_result[,class_label,drop=FALSE],by="row.names")
rownames(loaddata)=loaddata[,1]
loaddata=loaddata[2:ncol(loaddata)]
idx <- which(loaddata =='-Inf', arr.ind = TRUE)
loaddata[idx]=0

draw_heatmap(loaddata[!is.na(loaddata[class_label]),],test[['SYMBOL']],class_label,class_label,FALSE)
```

```{r}
setdiff(genes, test[['SYMBOL']])
```
...
cluster2_samples <- df.sample_cluster$kmeans_2_clusters == 2
cluster1_samples <- df.sample_cluster$kmeans_2_clusters == 1
...
# Calculate mean
mean_expr_cluster1 <- colMeans(loaddata[cluster1_samples, 1:(ncol(loaddata)-1)],na.rm=TRUE)
mean_expr_cluster2 <- colMeans(loaddata[cluster2_samples,1:(ncol(loaddata)-1)] ,na.rm=TRUE)

diff_expr <- mean_expr_cluster2 /mean_expr_cluster1

upregulated_genes <- colnames(loaddata[,1:(ncol(loaddata)-1)])[abs(diff_expr) > 1]
...
test = bitr(upregulated_genes, #数据集
  fromType="SYMBOL", #输入为SYMBOL格式
  toType="ENTREZID",  # 转为ENTERZID格式
  OrgDb="org.Hs.eg.db") #人类 数据库
...
kegg_table_draw
# kegg_table_draw=kegg_table_draw[(nrow(kegg_table_draw)-min((nrow(kegg_table_draw)-1),15)):nrow(kegg_table_draw),]
dirpath='~/workstation/MetabSubtype/tasks/MultiOmics/results/20240924_external_genes/'
write.csv(kegg_table,paste0(dirpath,'external_up.csv',collapse = '/'))
